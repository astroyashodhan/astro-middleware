...
// ── Normalize plan from any button label or trait value ───────────────
...
// ── Extract plan from ALL possible locations in Interakt payload ──────
function extractPlanFromPayload(body, dataArray) {
  // 1. From workflow data array (new users)
  const fromArray = extractField(dataArray, "getpredection");
  if (fromArray) return normalizePlan(fromArray) || fromArray;

  // 2. From short key body (returning users via custom webhook)
  if (body.tp) return normalizePlan(body.tp) || body.tp;

  // 3. From customer_user_traits (Interakt stores contact traits here)
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // ── Detect returning user (short key body has "pf") ───────────────
    ...
    if (isReturningUser) {
      ...
      addLog("RETURNING_RECEIVED", { phone: fullPhone, name, plan: planType, raw_plan: planRaw });
      ...
      const r = await axios.post(makeUrl, { phone_full: fullPhone, phone_number: finalPN, country_code: finalCC, name: finalName, dob: finalDob, birth_time: finalBirthTime, birth_place: finalBirthPlace, plan_type: planType }, { headers: { "Content-Type": "application/json" } });
      ...
      // ── Extract plan from ALL possible locations ───────────────────
      const planType = extractPlanFromPayload(body, dataArray);

      addLog("EXTRACTED", {
        name, dob, birthTime, birthPlace, planType,
        phone_full: fullPhone,
        // Log raw payload keys to help debug
      ...
      const r = await axios.post(makeUrl, { phone_full: fullPhone, phone_number: phoneNumber, country_code: countryCode, name: finalName, dob: finalDob, birth_time: finalBirthTime, birth_place: finalBirthPlace, plan_type: finalPlanType }, { headers: { "Content-Type": "application/json" } });
...
<style>* { box-sizing: border-box; } body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f0f0f; color: #e0e0e0; padding: 24px; margin: 0; } h1 { color: #a78bfa; margin-bottom: 4px; font-size: 22px; } h2 { color: #a78bfa; font-size: 16px; margin: 28px 0 10px; } .subtitle { color: #64748b; font-size: 13px; margin-bottom: 24px; } .stats { display: flex; gap: 16px; margin-bottom: 28px; flex-wrap: wrap; } .stat { background: #1a1a2e; border-radius: 10px; padding: 16px 24px; min-width: 140px; border: 1px solid #2a2a4a; } .stat .num { font-size: 32px; font-weight: 700; color: #a78bfa; } .stat .label { font-size: 12px; color: #64748b; margin-top: 4px; } .stat.green .num { color: #86efac; } .stat.yellow .num { color: #fcd34d; } .stat.red .num { color: #fca5a5; } .stat.blue .num { color: #93c5fd; } .stat.orange .num { color: #fdba74; } .stat.teal .num { color: #5eead4; } .stat.pink .num { color: #f9a8d4; } table { width: 100%; border-collapse: collapse; font-size: 13px; } th { text-align: left; padding: 10px 12px; background: #1a1a2e; color: #a78bfa; font-weight: 600; } td { padding: 10px 12px; border-bottom: 1px solid #1e1e2e; vertical-align: top; } tr:hover td { background: #1a1a2e; } .badge { padding: 3px 9px; border-radius: 20px; font-size: 11px; font-weight: 600; display: inline-block; } .success { background: #14532d; color: #86efac; } .error { background: #7f1d1d; color: #fca5a5; } .waiting { background: #78350f; color: #fcd34d; } .info { background: #1e3a5f; color: #93c5fd; } .returning { background: #3b1f6e; color: #d8b4fe; } .nav a { color: #a78bfa; text-decoration: none; margin-right: 20px; font-size: 13px; }</style></head>
...
<b>Detection</b>        → pf key present = returning user ✅<br>
<b>Plan extraction</b>  → checks data.data array + body.tp + customer_user_traits + data fields ✅<br>
<b>Plan normalize</b>   → "Get Today Prediction"→Prediction | "Ask a Question"→Ask | "Book 15 Min Call"→Consult ✅<br>
...
// ── Boot ──────────────────────────────────────────────────────────────
initDB().then(() => {
  app.listen(PORT, () => console.log(`🚀 Astro middleware v19 running on port ${PORT}`));
}).catch(err => {
