...
const MAKE_ASK_WEBHOOK_URL = 'https://hook.eu1.make.com/168u4w0lu9f7huxpqesu9xg2dsu4ou98';
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // ── ASK keyword flow (type sent from Interakt Calculate Values step) ──────
    if (body.type === "ask_keyword") {
      const phone_full = body?.pf || '';

      addLog("ASK_RECEIVED", { phone_full });

      if (!phone_full) {
        addLog("ASK_ERROR", { reason: "phone_full empty", body });
        return res.status(200).json({ status: "ask_no_phone" });
      }

      await axios.post(
        MAKE_ASK_WEBHOOK_URL,
        { phone_full },
        { headers: { "Content-Type": "application/json" } }
      );

      addLog("ASK_FORWARDED", { phone_full });
      return res.status(200).json({ status: "ask_forwarded" });
    }
  ...
  const rows = logs.map(log => `
    <tr>
      ...
      <td><span class="badge ${
        ...
        log.type.includes("FORWARDED") || log.type === "MAKE_RESPONSE" ? "success" :
        log.type === "WAITING" ? "waiting" : "info"
      }">${log.type}</span></td>
  ...
  res.send(`
    ...
    <body>
      ...
      <table>
        ...
        <tbody>${rows || '<tr><td colspan="3" style="color:#888">No events yet.</td></tr>'}</tbody>
      </table>
    </body>
    </html>
  `);
